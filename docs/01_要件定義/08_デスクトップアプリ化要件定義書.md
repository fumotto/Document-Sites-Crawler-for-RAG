...
- Inno Setupの`PrivilegesRequired=lowest`を用い、既定のインストール先を`{localappdata}\Programs\DocumentSitesCrawlerForRAG`とする。
...
| DATA-1 | データ保存先ルートパス | `%USERPROFILE%\DocumentSitesCrawlerForRAG\`（ユーザーフォルダ直下）。`%LOCALAPPDATA%`は採用しない |
...
| 項目 | 内容 |
|---|---|
| インストール先 | `{localappdata}\Programs\DocumentSitesCrawlerForRAG`（8.3節、管理者権限不要） |
  ...
  build-windows:
    ...
    steps:
      ...
      - name: Build EXE with PyInstaller
        run: |
          pyinstaller --noconsole --onedir --add-data "web;web" `
            --name DocumentSitesCrawlerForRAG src/app/gui/desktop_main.py
      ...
      - name: Create GitHub Release and upload asset
        ...
        with:
          files: Output/DocumentSitesCrawlerForRAGSetup.exe
...
- 内容：シンプルな単一HTMLページ。以下を含む：
  - アプリ名・簡単な説明
  - 「最新版をダウンロード」ボタン（GitHub Releasesの`latest`エイリアスURLへリンク：`https://github.com/{owner}/{repo}/releases/latest/download/DocumentSitesCrawlerForRAGSetup.exe`）
  - 13節のSmartScreen注意書き
  - GitHubリポジトリへのリンク
